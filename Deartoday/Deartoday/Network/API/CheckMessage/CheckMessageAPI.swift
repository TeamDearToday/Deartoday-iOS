...
final class CheckMessageAPI {
    ...
    public private(set) var checkMessageData: GeneralResponse<[String]>?
    
    // MARK: - GET
    
    func getCheckMessage(completion: @escaping (GeneralResponse<[String]>?) -> ()) {
        checkMessageProvider.request(.checkMessage) { result in
            switch result {
            case .success(let response):
                do {
                    self.checkMessageData = try response.map(GeneralResponse<[String]>?.self)
